                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
  ...
  mvMatches12.reserve(mvKeys2.size());// jin:大致的reserve
  ...
  for(size_t i=0, iend=vMatches12.size();i<iend; i++)
  {
    ...
    if(vMatches12[i]>=0)
    {
      ...
      mvMatches12.push_back(make_pair(i,vMatches12[i]));// jin:匹配关系变成了pair
			//标记参考帧1中的这个特征点有匹配关系
      mvbMatched1[i]=true;
    }
    else
  ...
  // 有匹配的特征点的对数
  const int N = mvMatches12.size();// jin:都是一一对应的关系
  ...
  // Step 2 在所有匹配特征点对中随机选择8对匹配特征点为一组，用于估计H矩阵和F矩阵
  // jin:共选择 mMaxIterations (默认200) 组，每组8个元素，每个元素代表的是mvMatches12中的索引
  ...
  for(int it=0; it<mMaxIterations; it++)
  {
    ...
    {
      // 随机产生一对点的id,范围从0到N-1
      int randi = DUtils::Random::RandomInt(0,vAvailableIndices.size()-1);// jin:随着随机采样的进行，会缩小
      ...
      vAvailableIndices[randi] = vAvailableIndices.back();// jin:机智！
...
void Initializer::FindHomography(vector<bool> &vbMatchesInliers, float &score, cv::Mat &H21)
{
  ...
  vector<cv::Point2f> vPn1, vPn2;
	// 记录各自的归一化矩阵
  cv::Mat T1, T2;
  Normalize(mvKeys1,vPn1, T1);// jin:返回做单位化所用的变换T
  ...
  // Perform all RANSAC iterations and save the solution with highest score
	//下面进行每次的RANSAC迭代
  // ! jin:这里RANSAC，就是先随机取8个点计算H，再用所有点来check误差，选择最好的H
  for(int it=0; it<mMaxIterations; it++)
  {
    // Select a minimum set
		// Step 2 选择8个归一化之后的点对进行迭代
    ...
    cv::Mat Hn = ComputeH21(vPn1i,vPn2i);// jin:计算单应矩阵
    ...
    float sigma)                        //估计误差
{
  ...
  // 基于卡方检验计算出的阈值（假设测量有一个像素的偏差）
	// 自由度为2的卡方分布，显著性水平为0.05，对应的临界阈值// jin:图像上是xy两个方向
  ...
  for(int i = 0; i < N; i++)// jin:注意这里是对所有匹配的特征对计算了重投影误差，而不限于ransac的那8个
  {
    // 一开始都默认为Inlier
    bool bIn = true;
    ...
    const float chiSquare1 = squareDist1 * invSigmaSquare;// jin:为了在不同层级上归一化

    // Step 2.3 用阈值标记离群点，内点的话累加得分
    if(chiSquare1>th) // todo 这跟统计上的卡方分布到底有什么关系啊？这个距离为什么可以作为卡方值，直接与阈值比较？
      bIn = false;    
    else
      // 误差越大，得分越低
      score += th - chiSquare1;// jin:只累积内点
    ...
    // 用阈值标记离群点，内点的话累加得分
    if(chiSquare2>th)
      bIn = false;// 因此，只要有一个匹配方向不满足，那该匹配对就不算内点
    else
      score += th - chiSquare2;   
                      ...
                      cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated, float minParallax, int minTriangulated)
{
  ...
  // ? jin:以上计算除了8组可能的R，t, n，具体理论还需要深入看
 ...
 */
// ! MVG中文版P82算法4.2
// todo 这里如何做的归一化了解了，但是还没有理解到底为什么这么做
void Initializer::Normalize(const vector<cv::KeyPoint> &vKeys, vector<cv::Point2f> &vNormalizedPoints, cv::Mat &T)                           //将特征点归一化的矩阵
{
  ...
  // Step 3 将x坐标和y坐标分别进行尺度归一化，使得x坐标和y坐标的一阶绝对矩分别为1
  // jin:根据统计量算，目前一阶绝对矩（变量到均值的期望，期望是用统计均值近似的）为meanDevX，那么所有变量都在去中心后除以这个值，使得一阶绝对矩为1 
  ...
  T.at<float>(1,2) = -meanY*sY;// jin:这是个相似变换，展开等价于以上去中心平移和xy拉伸操作
}
...
{   
  ...
  // Camera 2 Projection Matrix K[R|t] // jin:这里的Rt都是世界相对于相机的位姿，即第一帧相对于第二帧的位姿
  // 计算第二个相机的投影矩阵 P2=K*[R|t]
  cv::Mat P2(3,4,CV_32F);
    .copyTo(P2.rowRange(0,3).colRange(0,3));
  ...
  for(size_t i=0, iend=vMatches12.size();i<iend;i++)
  {
    ...
    const cv::KeyPoint &kp2 = vKeys2[vMatches12[i].second];
		//存储三维点的的坐标
    cv::Mat p3dC1;
    // ? jin:真正的做三角化的步骤:已知两个位姿和特征点坐标，求解3维点的齐次坐标
    // 利用三角法恢复三维点p3dC1
    ...
    float cosParallax = normal1.dot(normal2)/(dist1*dist2);// jin:视差角
    ...
    cv::Mat p3dC2 = R*p3dC1+t;// jin:Rt本来就代表的是世界（第一帧）相对于第二帧的位姿	
		//判断过程和上面的相同
    if(p3dC2.at<float>(2)<=0 && cosParallax<0.99998)// jin:在这里其实只是判断在第二帧中的深度是否为正
      continue;
 ...
 */
void Initializer::DecomposeE(const cv::Mat &E, cv::Mat &R1, cv::Mat &R2, cv::Mat &t) // ! 这个函数需要深入看
{
  ...
  // 左奇异值矩阵U的最后一列就是t，对其进行归一化 // ? jin:svd解方程用的是vt的最后一行
  u.col(2).copyTo(t);
  t=t/cv::norm(t);
